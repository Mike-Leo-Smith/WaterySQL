 ...
 * 
 * 还有非常重要的一点，BufType b = bpm->allocate_page(...)
 ...
 */
#include "bufmanager/BufferedPageManager.h"
...
int test_filesystem() {
    ...
    BufferedPageManager *bpm = new BufferedPageManager(fm);
    ...
    for (int pageID = 0; pageID < 1000; ++pageID) {
        int index;
        //为pageID获取一个缓存页
        BufType b = bpm->get_page(fileID, pageID, index);
        //注意，在allocPage或者getPage后，千万不要进行delete[] b这样的操作
        //内存的分配和管理都在BufPageManager中做好，不需要关心，如果自行释放会导致问题
        std::cout << b[0] << ":" << b[1] << std::endl;        //读取缓存页中第一个整数
        bpm->mark_access(index); //标记访问
        b = bpm->get_page(f2, pageID, index);
        std::cout << b[0] << ":" << b[1] << std::endl;
        bpm->mark_access(index);
    }
